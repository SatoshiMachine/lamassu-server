...
// ANCHOR: Lightning Network Types and Validation
...
// ANCHOR_END: Lightning Network Types and Validation
 ...
 * @description Customer buys Lightning, ATM sends payment
 * Flow:
 * 1. Customer provides empty invoice
 * 2. ATM sends payment for fiat amount
 * 3. Payment confirmed via status check
 */
function sendFundsLN(walletId, invoice, cryptoAtoms, token, endpoint) {
  // NOTE: Main function for sending Lightning payment to customer
  const sendLnNoAmount = {
...
function sendProbeRequest(walletId, invoice, cryptoAtoms, token, endpoint) {
  // NOTE: Probes payment route before sending
...
function newInvoice(walletId, cryptoAtoms, token, endpoint) {
  // NOTE: Creates invoice for customer to pay
  const createInvoice = {
...
function getInvoiceStatus(token, endpoint, address) {
  // NOTE: Monitors payment status of customer's Lightning payment
  const query = {
  ...
  return checkCryptoCode(cryptoCode)
    .then(() => {
      ...
      if (isLnInvoice(address)) {
        // NOTE: Lightning payment status check
