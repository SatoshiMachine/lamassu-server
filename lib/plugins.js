...
function plugins (settings, deviceId) {
  ...
  function getPhoneCode (recipient) {
    ...
    // Check if recipient is a Nostr public key (hex string of 64 chars)
    const isNostrPubkey = /^[0-9a-f]{64}$/.test(recipient)
    
    if (isNostrPubkey) {
      return nostr.sendSecurityCode(settings, recipient, code, timestamp)
    } else {
      // Regular phone number flow
      return sms.getSms(CONFIRMATION_CODE, recipient, { code, timestamp })
        .then(smsObj => {
          const rec = {
            sms: smsObj
          }
